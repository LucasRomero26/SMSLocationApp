package com.tudominio.smslocation.ui.theme

import androidx.compose.ui.graphics.Color

// Colores primarios - Tema moderno en azul/verde
val Primary = Color(0xFF1976D2)
val OnPrimary = Color(0xFFFFFFFF)
val PrimaryContainer = Color(0xFFD1E4FF)
val OnPrimaryContainer = Color(0xFF001D36)

// Colores secundarios
val Secondary = Color(0xFF535F70)
val OnSecondary = Color(0xFFEFEFEF)
val SecondaryContainer = Color(0xFFD7E3F7)
val OnSecondaryContainer = Color(0xFF101C2B)

// Colores terciarios - Verde para elementos de acción
val Tertiary = Color(0xFF4CAF50)
val OnTertiary = Color(0xFFFFFFFF)
val TertiaryContainer = Color(0xFFC8E6C9)
val OnTertiaryContainer = Color(0xFF1B5E20)

// Colores de error
val Error = Color(0xFFBA1A1A)
val ErrorContainer = Color(0xFFFFDAD6)
val OnError = Color(0xFFFFFFFF)
val OnErrorContainer = Color(0xFF410002)

// Colores de superficie
val Surface = Color(0xFFFDFCFF)
val OnSurface = Color(0xFF1A1C1E)
val SurfaceVariant = Color(0xFFDFE2EB)
val OnSurfaceVariant = Color(0xFF43474E)

// Colores de contorno
val Outline = Color(0xFF73777F)
val OutlineVariant = Color(0xFFC3C7CF)

// Colores específicos para modo oscuro
val SurfaceDark = Color(0xFF1A1C1E)
val OnSurfaceDark = Color(0xFFE2E2E6)
val SurfaceVariantDark = Color(0xFF43474E)
val OnSurfaceVariantDark = Color(0xFFC3C7CF)